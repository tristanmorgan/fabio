...
func (bgph *BGPHandler) Start() error {
	...
	if len(bgph.config.GOBGPDCfgFile) > 0 {
		initialCfg, err := oc.ReadConfigfile(bgph.config.GOBGPDCfgFile, "toml")
		if err != nil {
		...
		if err := s.StartBgp(context.Background(), &api.StartBgpRequest{
			Global: oc.NewGlobalFromConfigStruct(&initialCfg.Global),
		}); err != nil {
...
func ValidateConfig(config *config.BGP) error {
	...
	if len(config.GOBGPDCfgFile) > 0 {
		_, err := oc.ReadConfigfile(config.GOBGPDCfgFile, "toml")
		if err != nil {
